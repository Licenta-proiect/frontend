      ...
      <CardContent className="space-y-6 pt-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="space-y-2">
            ...
            <Input placeholder="Ex: Popescu" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="h-10 border-gray-200 focus-visible:ring-brand-blue/30" />
          </div>
          <div className="space-y-2">
            <Label className="text-sm font-bold text-gray-900">Prenume</Label>
            <Input placeholder="Ex: Ion" value={lastName} onChange={(e) => setLastName(e.target.value)} className="h-10 border-gray-200 focus-visible:ring-brand-blue/30" />
          </div>
          <div className="space-y-2">
            <Label className="text-sm font-bold text-gray-900">Email</Label>
            <Input type="email" placeholder="admin@usm.ro" value={email} onChange={(e) => setEmail(e.target.value)} className="h-10 border-gray-200 focus-visible:ring-brand-blue/30" />
          </div>
        </div>
        <Button onClick={handleAdd} className="bg-brand-blue hover:bg-brand-blue-dark text-white font-bold shadow-md transition-all active:scale-95">
